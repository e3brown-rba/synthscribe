...
class SynthScribeLogger:
    ...
    def __init__(self, name: str, log_dir: Optional[Path] = None, 
        ...
        if file_output and log_dir:
            ...
            log_file = log_dir / f"synthscribe_{datetime.utcnow().strftime('%Y%m%d')}.log"
            file_handler = logging.FileHandler(log_file)
            file_handler.setFormatter(StructuredFormatter())
            file_handler.setLevel(logging.DEBUG)
            self.logger.addHandler(file_handler)
    
    def log_request(self, user_id: str, mood: str, 
                   provider: str) -> str:
    ...
    def log_llm_call(self, request_id: str, provider: str, 
                    model: str, prompt_length: int):
        """Log LLM API call"""
        self.logger.debug(
            "Calling LLM API",
            extra={
    ...
    def log_llm_response(self, request_id: str, response_length: int, 
    ...
    def log_recommendation(self, request_id: str, user_id: str, 
                          recommendation_count: int, genres: list):
    ...
    def log_user_feedback(self, user_id: str, genre: str, 
                         rating: float):
    ...
    def performance(self, operation: str, 
                   metadata: Optional[Dict[str, Any]] = None) -> PerformanceLogger:
...
def get_logger(name: str = "synthscribe") -> SynthScribeLogger:
    ...
    if _logger is None:
        ...
        _logger = SynthScribeLogger(
            name=name,
            log_dir=log_dir,
            file_output=config.analytics.enabled
        )
...
if __name__ == "__main__":
    ...
    with logger.performance("generate_recommendations", 
                          {"request_id": request_id}):
        # Simulate some work
        time.sleep(0.1)
