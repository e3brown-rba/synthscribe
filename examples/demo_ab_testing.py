...
def main():
    ...
    if sig['winner']:
        winning_variant = next(v for v in results['variants'] if v['name'] == sig['winner'])
        baseline = next(v for v in results['variants'] if v['name'] == 'zero_shot')
        
        improvement = ((winning_variant['metrics']['conversion_rate'] - 
                       baseline['metrics']['conversion_rate']) / 
                      baseline['metrics']['conversion_rate'] * 100)
        
        print(f"\nDeploying '{sig['winner']}' variant would result in:")
        print(f"  • {improvement:+.1f}% improvement in user engagement")
        print(f"  • Better recommendation quality ({winning_variant['metrics']['avg_feedback_score']:.2f} vs {baseline['metrics']['avg_feedback_score']:.2f})")
